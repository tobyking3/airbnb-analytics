...
svgBar.selectAll("rect")
    ...
    .append("rect")
  .attrs({
        x: function(d, i) {return i * (wBar / dataset.length);},
        y: function(d) {return hBar - (d*4);},
        width: wBar / dataset.length - padding,
...
d3.csv("listings.csv").then(function(csv){
  ...
  var latPoint = parseFloat(csv[0].latitude);
  var longPoint = parseFloat(csv[0].longitude);
  ...
  d3.json("map.geojson").then(function(json){
    ...
    svgMap.selectAll("path")
      ...
      svgMap.selectAll('.circle-icon')
        ...
        .enter()
        .append('circle')
        .attr("cx", projection([longPoint, latPoint])[0])
        .attr("cy", projection([longPoint, latPoint])[1])
        .attr("r", "2px")
        .attr("fill", "red")
        .attr("class","points")
  });
